            ...
            <div className="block sm:hidden space-y-3">
              {filteredCustomers.map((cust, index) => {
                ...
                return (
                  <div
                    key={cust.id}
                    className="bg-[#FAF3FA] p-3.5 rounded-2xl border border-[#EEDBF0] space-y-3 shadow-2xs hover:border-[#682A6E]/40 transition-all"
                  >
                    {/* Top Row */}
                    <div className="flex items-center justify-between gap-2">
                      <div className="flex items-center gap-2.5 min-w-0">
                        <span className="text-[10px] font-bold text-[#7C637E] shrink-0">#{index + 1}</span>
                    ...
                    <div className="flex items-center justify-between pt-0.5">
                      <span className="text-[10px] text-[#7C637E] font-medium">Last: {cust.lastPurchase}</span>
                      <button
                        onClick={() => onOpenMessageModal?.(cust)}
                        className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full bg-[#2D142E] hover:bg-[#471E4A] text-white text-[11px] font-semibold transition-all shadow-2xs cursor-pointer"
                      >
                        <MessageCircle className="w-3.5 h-3.5 text-[#25D366] fill-[#25D366]/20 stroke-[2.5]" />
                        <span>Message</span>
                      </button>
                    </div>
                  </div>
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="py-2.5 px-2.5">
                          <div className="flex items-center gap-2">
                            <div className="w-7 h-7 rounded-full bg-[#3D1A40] text-[#F5CC96] border border-[#E4D2E6] shadow-2xs flex items-center justify-center font-serif text-xs font-bold shrink-0">
                              {cust.name ? cust.name.charAt(0).toUpperCase() : "C"}
                            </div>
